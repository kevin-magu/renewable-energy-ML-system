document.getElementById("generate-report").addEventListener("click", function() {
    ...
    setTimeout(function() {
        ...
        let previousResourceValue = resourceQuantity; // Initialize with the user input
        // Generate resource comparison report for each year
        for (let year = 2025; year <= selectedYear; year++) {
            // Simulate an increasing trend for predicted values
            const predictedResourceValue = previousResourceValue + Math.random() * 10;
            previousResourceValue = predictedResourceValue;
            ...
            if (predictedResourceValue === null) {
               ...
               reportContent.innerHTML += `<p>Year ${year}: ${resourceNeeded} ${resourceType} needed</p>`;
            }
        }
